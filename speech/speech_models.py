...
def style_model(net, num_styles, activation_fn=tf.nn.relu, EMBEDDING_SIZE = 8192):
  #net = slim.fully_connected(net, num_outputs=EMBEDDING_SIZE, scope='style_fc1', activation_fn=activation_fn)
  net = slim.fully_connected(net, num_outputs=num_styles, scope='style_fc2', activation_fn=tf.identity)
  return net


def label_model(net, style_net, num_labels, activation_fn=tf.nn.relu, EMBEDDING_SIZE = 1024):
  print (tf.get_variable_scope().name)
  # dropping concat, see my diary (sec: Concatenation trouble with Speech code [21st May]) for a more elaborate comment.
  # experiments that were reported before April 19th 2019 had the following line commented, which is to ignore the passed style net, this is undesired, most probably the numbers would only get better when concatenating.
  # for ucgv8 and rest of the methods that pass a vector in latent dimension which is typically small, then guessing it does not make much difference when concatenating a vector of dimension say to a 7160 dimension label embedding.
  # surprisingly the network does not train when concatenated with the style vector. This is the case even when I stop_gradient on style net.
  # However pasing it through another linear layer before concat helps.
  # net = tf.concat(axis=1, values=[net, tf.stop_gradient(style_net)])
  # style_fcnet = slim.fully_connected(style_net, num_outputs=EMBEDDING_SIZE, scope='label_fc1', activation_fn=activation_fn)
  # net = tf.concat(axis=1, values=[net, style_fcnet])
  net = slim.fully_connected(net, num_outputs=num_labels, scope='label_fc2', activation_fn=tf.identity)
  return net


def descriminator(net, num_styles, EMBEDDING_SIZE=8192):
    with tf.variable_scope('DAN'):
        ...
        net = slim.fully_connected(net, EMBEDDING_SIZE//2, scope='desc3', activation_fn=fun)
        net = slim.fully_connected(net, EMBEDDING_SIZE//2, scope='desc4', activation_fn=fun)
        net = slim.fully_connected(net, num_styles, scope='desc5', activation_fn=tf.identity)
        logits = net
    return logits
...
def create_model(fingerprint_input, model_settings, model_architecture,
                 is_training, runtime_settings=None, scope='model'):
  ...
  Returns:
    (Only 'conv' arch type is handled)
    TensorFlow node outputting logits results, and endpoints object which contains the 
  handle to dropout placeholder and embeddings
  ...
  if model_architecture == 'single_fc':
    ...
    return create_conv_model(fingerprint_input, model_settings, is_training, scope)
...
def create_conv_model(fingerprint_input, model_settings, is_training, scope):
  ...
  with tf.variable_scope(scope):
    ...
    first_weights = tf.get_variable("first_weights", [first_filter_height, first_filter_width, 1, first_filter_count], 
			initializer=tf.truncated_normal_initializer(stddev=0.01))
    first_bias = tf.get_variable("first_bias", [first_filter_count], initializer=tf.zeros_initializer())
    ...
    second_weights = tf.get_variable("second_weights", [second_filter_height, second_filter_width, first_filter_count, second_filter_count], 
			initializer=tf.truncated_normal_initializer(stddev=0.01))
    second_bias = tf.get_variable("second_bias", [second_filter_count], initializer=tf.zeros_initializer())
