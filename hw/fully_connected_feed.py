...
def fill_feed_dict(x_ys, images_pl, labels_pl):
  ...
  # `batch size` examples.
  def next_batch(_xs, _ys, bs):
    if len(_xs) < bs:
      bs = len(_xs)
    idxs = np.random.choice(len(_xs), size=bs, replace=False)
    return _xs[idxs], _ys[idxs]
  
  images_feed, labels_feed = next_batch(xs, ys, FLAGS.batch_size)
...
def run_training():
  ...
  for ti in tqdm.tqdm(range(len(train_images)), desc="Transforming train images"):
    train_images[ti] = rot(train_images[ti], FLAGS.train_angle, reshape=False)

  for ti in tqdm.tqdm(range(len(test_images)), desc="Transforming test images"):
    test_images[ti] = rot(test_images[ti], FLAGS.test_angle, reshape=False)
    
  val_frac = int(0.1*len(train_images))
  val_images, val_labels = train_images[-val_frac:], train_labels[-val_frac:]
  train_images, train_labels = train_images[:-val_frac], train_labels[:-val_frac]

  train, test, validation = (train_images, train_labels), (test_images, test_labels), (val_images, val_labels)
  ...
  with tf.Graph().as_default():
    ...
    logits, net = mnist.inference(images_placeholder)
    ...
    _vars = [_var for _var in _vars if _var.name.find('tune')<0]
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      ...
      type=float,
      default=0,
      help='Training angle (in degrees)'
  )
  parser.add_argument(
      '--test_angle',
      type=float,
  ...
  FLAGS.log_dir = "mnist_log/%d_%d" % (FLAGS.train_angle, FLAGS.test_angle)
